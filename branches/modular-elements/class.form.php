...
class form extends pfbc { 
	...
	private function attachElement($params) {
		...
		else if ($eleType == "captcha") {
                    include_once('includes/elements/captcha.php');
                    global $captcha;
...
STR;
		...
		$_SESSION["pfbc-instances"][$this->attributes["id"]] = serialize($this);
                $_SESSION["pfbc-instances"]['captcha'] = serialize($captcha);
	...
	public function validate() {
		...
		if(!empty($_SESSION["pfbc-instances"]) && array_key_exists($this->attributes["id"], $_SESSION["pfbc-instances"])) {
			...
			$form = unserialize($_SESSION["pfbc-instances"][$this->attributes["id"]]);
                        if(get_class($form)=='captcha'){

                            $captcha = unserialize($_SESSION["pfbc-instances"]['captcha']);
                            return $captcha->phpvalidation($referenceValues);
...
STR;
	...
	public function renderJS($returnString=false) {
		...
		if(!empty($_SESSION["pfbc-instances"]) && array_key_exists($this->attributes["id"], $_SESSION["pfbc-instances"])) {
			...
			if(empty($form->preventCaptchaLoad) && !empty($captcha->captchaID)) {
				if($form->https)
...
STR;
			...
			if(!empty($captcha->captchaID)) {
                                $str .= $captcha->javascript();
			}
